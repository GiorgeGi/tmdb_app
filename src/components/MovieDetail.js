...
export default function MovieDetail() {
          ...
          return (
            <div className="card mb-2" key={idx}>
              <div className="card-body">
                ...
                {r.url && (
                  <p className="mb-0">
                    <small><a href={r.url} target="_blank" rel="noopener noreferrer">Read full review</a></small>
                  </p>
                )}
              </div>
            </div>
  ...
  const handleAction = async (action) => {
    ...
    try {
      const res = await fetch("http://localhost/tmdb_app/api/update_list.php", {
        ...
        body: JSON.stringify({ type: "movie", tmdb_id: movie.id, action })
      });
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const [movieData, regionsData, providers, reviewsData, creditsData] = await Promise.all([
          ...
          fetchJson(`https://api.themoviedb.org/3/movie/${id}/credits?api_key=${apiKey}&language=en-US`)
        ]);
    ...
    <>
      ...
      <div className="container mt-4">
        ...
        <div className="mb-4">
          ...
          <div className="d-flex flex-wrap gap-3">
            {paginatedActors.map(actor => (
              <div key={actor.cast_id} style={{ textAlign: "center", width: "100px" }}>
                <img
        ...
        <div className="mb-3 px-3">
          ...
          <div className="form-text text-muted" id="providers-block">{renderProviders()}</div>
        </div>
